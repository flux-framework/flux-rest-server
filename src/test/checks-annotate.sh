...
for trs in $(find . -name '*.trs'); do
    ...
    if test "$result" != "PASS" -a "$result" != "SKIP"; then
        ...
        catfile ${testbase}.broker.log >> $logfile
        : $((errors++))
    fi
done
...
#
#  Only meaningful for an in-tree `make check`: a distcheck build runs the
#  testsuite under its own build subdirectory, so ./t holds no *.trs and every
#  test would be reported missing.
#
if ls t/*.trs >/dev/null 2>&1; then
    ls -1 t/*.t | sort >/tmp/expected.$$
    ls -1 t/*.trs | sed 's/rs$//' | sort >/tmp/actual.$$
    comm -23 /tmp/expected.$$ /tmp/actual.$$ >/tmp/missing.$$
    if test -s /tmp/missing.$$; then
        error "Detected $(wc -l </tmp/missing.$$) missing tests:"
        for f in $(cat /tmp/missing.$$); do
            printf "$f\n"
            file=${f//.t}
            catfile ${file}.log
            catfile ${file}.output
    ...
    rm -f /tmp/expected.$$ /tmp/actual.$$ /tmp/missing.$$
fi
